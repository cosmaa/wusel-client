<template>
    <svg xmlns="http://www.w3.org/2000/svg"
        ...
        <g class="links" id="l-links">
            <path v-for="link in links" :d="linkPath(link)" :id="link.id"
        ...
        <g class="nodes" id="l-nodes" v-if='!noNodes'>
            <template v-for='(node,key) in nodes'>
                ...
                <circle v-else
                        ...
                        v-bind='node._svgAttrs'></circle>
            </template>
        </g>

        <g class="labels" id="link-labels" v-if='linkLabels'>
            <text class="link-label" v-for="link in links" :font-size="fontSize">
                <textPath v-bind:xlink:href="'#' + link.id" startOffset="50%">{{ link.name }}</textPath>
            </text>
        </g>

        <g class="labels" id="node-labels" v-if="nodeLabels">
            <text class="node-label" v-for="node in nodes"
